...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--use_analytics', action='store_true', help='Fetch YouTube Analytics API data')
    arg_parser.add_argument('--start_date', type=str, default='2024-01-01', help='Start date for analytics data (YYYY-MM-DD)')
    arg_parser.add_argument('--end_date', type=str, default='2024-12-31', help='End date for analytics data (YYYY-MM-DD)')
    ...
    if args.use_analytics:
        analytics_results = fetch_analytics_for_videos(video_ids, args.start_date, args.end_date)
        for video_id, analytics_data in analytics_results.items():
            for key, val in analytics_data.items():
                df.loc[video_id, f"analytics_{key['name']}"] = val
...
def get_authenticated_analytics_service(client_secrets_file: str = "client_secrets.json"):
    creds = None
    if os.path.exists("token.json"):
        creds = Credentials.from_authorized_user_file("token.json", YT_ANALYTICS_SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(client_secrets_file, YT_ANALYTICS_SCOPES)
...
def fetch_analytics_for_videos(video_ids: list[str], start_date: str, end_date: str):
    ...
    for video_id in video_ids:
        try:
            ...
            if response.get("rows"):
                ...
                analytics_data[video_id] = dict(zip(response["columnHeaders"], metrics))
        except Exception as e:
            logging.warning(f"Analytics fetch failed for video {video_id}: {e}")
